...
router.post('/add', async (req, res) => {
  ...
  if (stubble) {
    ...
    const new_stubble = new Stubble({ stubbleType, farmers: [{ number, weight }] })
...
router.post('/calculate', async (req, res) => {
  ...
  const stubble = await Stubble.findOne({ stubbleType })
  const shares = utils.calculate_share(total_cost, convinience_charge, stubble.farmers)
...
router.get('/getStubbles', async (req, res) => {
  ...
  stubbles.forEach(stubble => {
    ...
    allStubbles.push[{ stubbleType:stubble.stubbleType, totalWeight, stubbleID:stubble._id }]
  })
  res.send(allStubbles);
})
